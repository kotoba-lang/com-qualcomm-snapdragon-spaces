(ns qualcomm-snapdragon-spaces.main
  "Kotodama WASM entrypoint for the Qualcomm Snapdragon Spaces clean-room actor (L4) — Clojure port.
  ...
  facts (`qualcomm_snapdragon_spaces.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "qualcomm_snapdragon_spaces")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"qualcomm_snapdragon_spaces.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "qualcomm_snapdragon_spaces-compat" :tier tier :entities entities} 200])
...
(defn start! [] :qualcomm_snapdragon_spaces-compat/ready)
