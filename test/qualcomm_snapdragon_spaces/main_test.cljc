(ns qualcomm-snapdragon-spaces.main-test
  "Contract + behavioral test for the qualcomm_snapdragon_spaces-compat L4 actor (cljc port).
  ...
  (:require [clojure.test :refer [deftest is testing run-tests]]
            ...
            [qualcomm-snapdragon-spaces.main :as m]))
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "qualcomm_snapdragon_spaces-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'qualcomm-snapdragon-spaces.main-test)]
